...
def create_sequences(source,
                     target,
                     SEQ_LENGTH = 30,
                     RANDOM_STATE = 1,
                     MIRROR = 1):
    #load dataset
    dataset = pickle.load(open(source, 'rb'))
    #create sequences object
    seqObj = Sequences(source,
    ...
    if MIRROR: train_df = train_with_mirror
    ...
    pickle.dump(seqObj, open(target, 'wb'))
...
if __name__ == '__main__':
    ...
    current_dataset = 'CONTRA_PSR.p'
    #create_sequences(source + current_dataset, target + current_dataset)
    #for debugging
    pass
